...
class Encoder(torch.nn.Module):
    ...
    # The goal is to collapse the feature map into a 2D tensor [batch_size, num_features]
    def forward(self, x):
        # adds an extra dimension to the tensor at position 1
        # the Conv2d layer expects a 4D tensor with shape [batch_size, channels, height, width]
        x = x.unsqueeze(1)

        x = self.cnn1(x)
        x = self.efficientnet(x)

        # squeeze(3) removes the last dimension if it's 1, and squeeze(2) removes the third dimension if it's 1.
